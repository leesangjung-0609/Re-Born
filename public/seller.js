...
const sellerId = urlParams.get('id'); // 예: seller.html?id=5
...
async function loadDataAndRender(endpoint, containerId, type) {
    ...
    if (!sellerId) {
        container.innerHTML = '<p style="padding: 20px; color: red;">판매자 정보를 찾을 수 없습니다.</p>';
        return;
    }
    ...
    try {
        ...
        const finalUrl = `${endpoint}${separator}userId=${sellerId}`;
